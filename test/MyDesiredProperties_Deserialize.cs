...
{
    [Fact(DisplayName = "No.9 引数:\"desiredProperties\"がJSON形式以外")]
    public void Case009()
    {
        Assert.ThrowsAny<Exception>(() => MyDesiredProperties.Deserialize("invalid format value"));
    }

    [Fact(DisplayName = "No.10 引数:\"desiredProperties\"がJSON形式")]
    public void Case010()
    {
        var ret = MyDesiredProperties.Deserialize("{}");
        Assert.NotNull(ret);
        Assert.IsType<MyDesiredProperties>(ret);
    }
