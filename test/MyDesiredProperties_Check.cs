...
public class MyDesiredProperties_Check
{
    private readonly ITestOutputHelper _output;

    public MyDesiredProperties_Check(ITestOutputHelper output)
    {
        _output = output;
    }

    [Fact(DisplayName = "No.11 プロパティ\"input_name\"がnull")]
    public void Case011()
    {
        var props = MyDesiredPropertiesCreater.Create();
        props.input_name = null;

        Assert.ThrowsAny<Exception>(() => props.Check());
    }

    [Fact(DisplayName = "No.12 プロパティ\"input_name\"が空文字")]
    public void Case012()
    {
        var props = MyDesiredPropertiesCreater.Create();
        props.input_name = "";
    ...
    [Fact(DisplayName = "No.13 プロパティ\"input_name\"がnull/空文字以外")]
    public void Case013()
    {
        var props = MyDesiredPropertiesCreater.Create();
        props.input_name = "test-no13--input";
        _output.WriteLine($"input_nameに設定するテスト値: {props.input_name}");

        props.Check();
    }

    [Fact(DisplayName = "No.14 プロパティ\"processes\"の要素数が0")]
    public void Case014()
    {
        var props = MyDesiredPropertiesCreater.Create();
        props.input_name = "test-no14--input";
        props.processes = [];

        Assert.ThrowsAny<Exception>(() => props.Check());
    }

    [Fact(DisplayName = "No.15 プロパティ\"processes\"の要素数が2")]
    public void Case015()
    {
        var props = MyDesiredPropertiesCreater.Create();
        props.input_name = "test-no15--input";
        props.processes = Enumerable.Range(0, 2)
            .Select(i => MyDesiredPropertiesCreater.CreateValidProcess($"dummy-process-{i}"))
            .ToList();
